...
import openglCommon.noise.ImprovedPerlinNoise;
...
public class Noise {	
	...
	public Noise(int channels, int size) {	
		image = Buffers.newDirectByteBuffer(size*size*size*channels);
	...
	double PerlinNoise_3D(int channels, int x, int y, int z)	{
        ...
        for (int i = 0; i < channels; i++) {
        	image.put((byte) (ImprovedPerlinNoise.noise(x * frequency, y * frequency, z * frequency) * amplitude));
	...
	private float smoothNoise3(int x, int y, int z) {
		float corners = ( 		noise3(x-1,y-1,z-1)+ 
								...
								noise3(x+1,y-1,z+1)+ 
								noise3(x-1,y+1,z+1)+ 
								noise3(x+1,y+1,z+1)) / 32f; 
		
		float sides   = (		noise3(x-1, y, z) +
				                noise3(x+1, y, z) +
				                noise3(x, y-1, z) +
				                noise3(x, y+1, z) +
				                noise3(x, y, z-1) +
	...
	private float smoothNoise(int octave, int x, int y) {
		float corners = ( 	noise2(octave, x-1, y-1)+
							noise2(octave, x+1, y-1)+
							noise2(octave, x-1, y+1)+
							noise2(octave, x+1, y+1)) 	/ 16;
		
		float sides   = ( 	noise2(octave, x-1, y)+
							noise2(octave, x+1, y)+
							noise2(octave, x, y-1)+
							noise2(octave, x, y+1)) 	/ 8;
	...
	private float cosInterpolate(float a, float b, float x) {
		...
		float f = (1f - (float)Math.cos((double)ft)) * .5f;
