...
public class MulticolorText extends Model {
    ...
    private HashMap<Integer, Color4> colors;
    ...
    public MulticolorText(Material material) {
        ...
        colors = new HashMap<Integer, Color4>();
    ...
    public void setString(GL3 gl, Program program, TypecastFont font, String str, Color4 basicColor,
            HashMap<Color4, String> colorSubStrings, int fontSize) {
        colors = new HashMap<Integer, Color4>();
        for (int i = 0; i < str.length(); i++) {
            colors.put(i, basicColor);
        }

        for (Map.Entry<Color4, String> entry : colorSubStrings.entrySet()) {
            Color4 color = entry.getKey();
        ...
        if (str.compareTo(cachedString) != 0) {
            ...
            List<VecF4> vertexColors = new ArrayList<VecF4>();
            for (int i = 0; i < glyphs.size(); i++) {
                GlyphShape glyph = glyphs.get(i);
                Color4 glypColor = colors.get(i);
            ...
            GLSLAttrib cAttrib = new GLSLAttrib(this.vertexColors, "MCvertexColors", GLSLAttrib.SIZE_FLOAT, 4);
