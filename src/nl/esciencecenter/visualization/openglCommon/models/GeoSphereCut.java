 ...
 * divisions. This sphere is cut, a quarter of it is drawn at a lower radius to
 * the center.
 * 
...
public class GeoSphereCut extends Model {
     ...
     */
    public GeoSphereCut(int latRibs, int lonRibs, float radius, boolean texCoordsIn3D) {
        ...
        makeVertices(points4List, normals3List, tCoords3List, radius, (radius * 0.75f));
     ...
     * @param radius
     *            the smaller radius of the cut part of the sphere.
     */
    private void makeVertices(List<VecF4> pointsList, List<VecF3> normalsList, List<VecF3> tCoords3List,
            float fullRadius, float cutRadius) {
        ...
        for (int lon = 0; lon < lonRibs; lon++) {
            ...
            float radius = fullRadius - (flon / flonribs) * (fullRadius - cutRadius);
