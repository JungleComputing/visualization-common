package nl.esciencecenter.visualization.openglCommon.datastructures;
...
import nl.esciencecenter.visualization.openglCommon.textures.HDRRBOTexture;
import nl.esciencecenter.visualization.openglCommon.textures.HDRTexture2D;


public class HDRFBO {
    private final IntBuffer     fboPointer;
    private final IntBuffer     rboPointer;
    private final HDRRBOTexture rboTexture;

    private final int           width, height;
    private boolean             initialized = false;

    public HDRFBO(int width, int height, int glMultitexUnit) {
        this.width = width;
        this.height = height;
        ...
        rboTexture = new HDRRBOTexture(width, height, glMultitexUnit);
    }
    ...
    public void init(GL3 gl) {
        try {
            ...
            rboTexture.init(gl);
            rboTexture.use(gl);
            gl.glFramebufferTexture(GL3.GL_DRAW_FRAMEBUFFER,
                    GL3.GL_COLOR_ATTACHMENT0, rboTexture.getPointer(), 0);
            ...
            gl.glFramebufferTexture(GL3.GL_DRAW_FRAMEBUFFER,
                    GL.GL_COLOR_ATTACHMENT0, rboTexture.getPointer(), 0);
    ...
    public HDRTexture2D getTexture() {
        return rboTexture;
    }
